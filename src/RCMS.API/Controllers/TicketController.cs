...
namespace RCMS.API.Controllers
{
    ...
    public class TicketController : ControllerBase
    {
        ...
        /// <summary>ایجاد تیکت</summary>
        [HttpPost]
        public async Task<IActionResult> Create([FromBody]TicketDto dto) => Ok(new { id = await _svc.CreateAsync(dto, 1, 1) });

        /// <summary>ویرایش تیکت</summary>
        [HttpPut]
        public async Task<IActionResult> Update([FromBody]TicketDto dto) => (await _svc.UpdateAsync(dto, 1, 1)) ? Ok() : NotFound();

        /// <summary>حذف منطقی تیکت</summary>
        [HttpDelete("{id}")]
        public async Task<IActionResult> Delete(long id) => (await _svc.DeleteAsync(id, 1, 1)) ? Ok() : NotFound();

        /// <summary>دریافت تیکت با شناسه</summary>
        [HttpGet("{id}")]
        public async Task<IActionResult> GetById(long id) { var x = await _svc.GetByIdAsync(id, 1); return x==null? NotFound(): Ok(x); }

        /// <summary>فهرست صفحه‌بندی‌شده تیکت</summary>
        [HttpGet]
